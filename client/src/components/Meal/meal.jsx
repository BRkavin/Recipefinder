...
const Meal = () => {
  ...
  const addToWishlist = async () => {
    try {
      const userEmail = localStorage.getItem('userEmail');
      if (!userEmail) throw new Error('User is not logged in.');

      const token = localStorage.getItem('token');
      if (!token) throw new Error('Token not found. User may not be authenticated.');
  ...
  const removeFromWishlist = async () => {
    try {
      ...
      const response = await fetch(`https://recipefinder-52yd.onrender.com/api/recipes/remove/${meal.idMeal}`, requestOptions);
      if (!response.ok) throw new Error(`Failed to remove from wishlist. Server responded with status: ${response.status}`);
  ...
  const isInWishlist = () => wishlist.some(item => item.idMeal === meal.idMeal);
  ...
  return (
    <section id='single-meal' className='meal-section'>
      <button className='back-button' onClick={() => navigate(-1)}>Back</button>
      {meal.strMeal ? (
          ...
          <div className="meal-content">
            ...
            <div className='meal-details'>
              <div className='meal-info'>
                <p><strong>Category:</strong> {meal.strCategory}</p>
                <p><strong>Area:</strong> {meal.strArea}</p>
                <p className="meal-yield">Yield: 6-8 servings</p>
                <p className="meal-prep-time">Prep Time: 5 min</p>
                <p className="meal-total-time">Total Time: 10 min</p>
                <h3>Ingredients:</h3>
                <ul>
                  {Object.keys(meal)
                    .filter(key => key.includes('Ingredient') && meal[key])
